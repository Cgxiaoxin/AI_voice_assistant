...
class WakeWordJieba:
    ...
    def process_text_and_send_signal(self, text):
        # 处理逻辑
        self.expressions_server.send("text_person", text)
        try:
            # 检测文本中是否包含“放”或“拿”或“抓”
            keywords = ['放', '拿', '抓', '到', '进', '入']
            found_keyword = None
            ...
            if not found_keyword:
                ...
                if '停' in text:
                    ...
                    self.expressions_server.send("img", "happy")
                    self.expressions_server.send("text_robot", "检测到停止指令，停止操作中")
                    text_to_speech("好的，现在开始停止执行操作")
                    return
                else:
                    # 不支持的功能
                    self.store.setValues(3, 1, [406])
                    self.expressions_server.send("img", "sad")
                    self.expressions_server.send("text_robot", "不好意思我没有听清，请重新唤醒我")
                    text_to_speech("不好意思我没有听清，请重新唤醒我")
                    self.is_awake = False
                    return

            # 根据检测到的关键动词开始拆分
            parts = text.split(found_keyword)
            # 前半部分分词并匹配
            print("正在拆分前面句子")
            ...
            if '盒子' in parts[0]:
                ...
                self.store.setValues(3, 1, [406])
                self.expressions_server.send("img", "sad")
                self.expressions_server.send("text_robot", "暂不支持此功能，请重新唤醒我")
            ...
            if matched_front == 'item1' and matched_back == 'item3':
                ...
                self.expressions_server.send("text_robot", "执行指令把" + "绿色方块" + "放到" + "方形" + "盒子里")
                self.expressions_server.send("img", "happy")
                text_to_speech("好的开始行动:把" + "绿色方块" + "放到" + "方形" + "盒子里")
                logger.info("发送信号: 401")
                logger.info("切换表情：开心")
            elif matched_front == 'item1' and matched_back == 'item4':
                self.store.setValues(3, 1, [402])
                self.expressions_server.send("text_robot", "执行指令把" + "绿色方块" + "放到" + "圆形" + "盒子里")
                self.expressions_server.send("img", "happy")
                text_to_speech("好的开始行动:把" + "绿色方块" + "放到" + "圆形" + "盒子里")
                logger.info("发送信号: 402")
                logger.info("切换表情：开心")
            elif matched_front == 'item2' and matched_back == 'item3':
                self.store.setValues(3, 1, [403])
                self.expressions_server.send("text_robot", "执行指令把" + "红色圆形" + "放到" + "方形" + "盒子里")
                self.expressions_server.send("img", "happy")
                text_to_speech("好的开始行动:把" + "红色圆形" + "放到" + "方形" + "盒子里")
                logger.info("发送信号: 403")
                logger.info("切换表情：开心")
            elif matched_front == 'item2' and matched_back == 'item4':
                self.store.setValues(3, 1, [404])
                self.expressions_server.send("text_robot", "执行指令把" + "红色圆形" + "放到" + "圆形" + "盒子里")
                self.expressions_server.send("img", "happy")
                text_to_speech("好的开始行动:把" + "红色圆形" + "放到" + "圆形" + "盒子里")
                ...
                self.store.setValues(3, 1, [406])
                self.expressions_server.send("img", "sad")
                self.expressions_server.send("text_robot", "抱歉，我没有听清，请重新唤醒我")
                text_to_speech("抱歉，我没有听清，请重新唤醒我")
...
# 主函数
async def main():
    detector = WakeWordJieba() # 初始化唤醒词识别
    executor = ThreadPoolExecutor(max_workers=3)  # 创建一个线程池

    try:
        await detector.run_modbus_server() # 启动Modbus服务器
        while True:
            # 启动唤醒检测
